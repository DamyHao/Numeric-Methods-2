...
nos puede proporcionar una primera aproximación al valor de la 
...
% En este archivo solo se hará el proceso de encontrar el 0 mediante
% Newton. La definición de la función a la que buscaremos el 0 es:
%{
...
enunciado pide encontrar donde se supera el 5%. Así ya preparamos la
función para el metodo de Newton:
...
% Visualizacion de la funcion a la que buscaremos la raíz mediante Newton:
ys = [];
titu = [0:0.01:pi];
for i = titu
    ys = [ys fa_funcio(i)];
...
% Hacemos la exploración de Newton entre 0 y pi, sin incluir este porque
...
Ara ens disposem a comprobar l'error de la cuadratura de Féjer quan ens
acostem a pi segons el nombre de punts utilitzats:
A l'eix X hi posarem les m per les quals evaluem i al Y hi posarem el
error maxim dels ultims punts més propers a pi ([pi-pi/2:10^-2:pi-10^-15]).
%}
...
aquesta integral NO és convergent, ja que si l'angle des del
